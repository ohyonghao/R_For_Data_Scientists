...
We see that the group 41-70 has the most cases, though it is the largest bucket other than
71+.
...
```{r}
pop.agg %>%
        filter( NAME %in% c("New York", "California") ) %>%
        group_by(NAME) %>%
        summarize(Population = sum(POPESTIMATE2018,na.rm=TRUE))
```

Though California is twice the size of New York by population we see that with California's
earlier action to shut down and stay at home that the curve has been succesfully flatenned
as compared to New York which delayed the order at first. This can be seen by the slow
growth rate which has not taken on the classic S-curve shape that logrithmic functions would,
whereas New York shows this quite distinctly.
